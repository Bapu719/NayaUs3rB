...
async def load_reload(event):
    ...
    try:
        ...
        msg = await event.respond(f"Done (re)loaded plugin {shortname}")
    ...
    if event.reply_to_msg_id:
        try:
            ...
            if "(" not in downloaded_file_name:
                ...
                await event.edit("Kardiya re install `{}`".format(os.path.basename(downloaded_file_name)))
            else:
                os.remove(downloaded_file_name)
                await event.edit("Errors! kuch locha ho gya.")
        except Exception as e:  # pylint:disable=C0103,W0703
